...
/**
 * @desc    Get public order by ID (for guest tracking)
 * @route   GET /api/orders/track/:id
 * @access  Public
 */
const getPublicOrderById = async (req, res, next) => {
  try {
    const order = await Order.findById(req.params.id)
      .populate('items.product');
    ...
    // For public access, we might want to limit sensitive info, 
    // but for now returning full order as requested for tracking
    res.json({
      success: true,
      data: order,
    });
